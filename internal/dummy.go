...
func (d *DummyRuntime) Run(ctx context.Context, step *schema.RunStep, prefix string) (map[string]string, error) {
	fmt.Printf(">>> RUN: %s (image=%s, cmd=%q)\n", step.Name, step.Image, step.Cmd)
	return nil, nil
}

func (d *DummyRuntime) Build(ctx context.Context, step *schema.BuildStep, prefix string) (map[string]string, error) {
